...
- **What:** iPad-native CRM + AI design studio for a single boutique business in India. **NOT YET IN USE — pre-pilot.** Verified against the production DB 2026-08-01: the only rows are seed data from 25–26 May, and `design_renders`, `design_tryons`, `job_cards`, `order_locks` and `reminder_log` are all **empty**. No render, try-on, job card, lock or reminder has ever completed. Treat every flow as unexercised until proven otherwise — "it compiles" and "it has ever worked" are different claims here. Designer workflow: sketch/reference → AI render → customer virtual try-on → job card for the karigar → GST invoice. Customers never touch the iPad; they receive WhatsApp messages.
...
- **GSTIN:** 15-char format **with a mod-36 check character** — `GSTINValidator` verifies both, at form submit. Empty is allowed (invoices just disabled). The checksum was added 2026-08-01 after a structure-only validator let a placeholder (`07AAAAA0000A1Z5`, seeded by migration 0023) pass as real, so the app enabled invoicing and generated GST invoices carrying a fabricated tax ID. Structure alone cannot catch that. Note it still does not prove the GSTIN is *registered* — only the GSTN portal does.
...
| Store privacy answers · release steps | `docs/DATA_HANDLING.md`, `docs/RELEASE_CHECKLIST.md` |
